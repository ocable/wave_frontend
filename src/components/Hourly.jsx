export default function Hourly({ timeIncrement, setTimeIncrement, hour, meridiem, sig_height, sig_period, sig_dir, wind_speed, wind_dir }) {
  return (
    <div className="flex flex-row h-14 drop-shadow-md bg-blue-300 rounded-3xl mt-6 mb-2">
      <section className="flex flex-row">
        <h3 className="font-radio font-normal text-3xl text-highlight self-center ml-4">{hour}</h3>
        <p className="font-radio font-normal text-xs text-highlight self-center mt-3 ml-1">{`${meridiem === 'AM' ? "am" : "pm"}`}</p>
